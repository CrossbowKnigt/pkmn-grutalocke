  ...
  def pbStartScene
    ...
    if pbInSafari?
      ...
      if pbBugContestState.lastPokemon
        content.push(_INTL("Capturas: {1}", PBSpecies.getName(pbBugContestState.lastPokemon.species)))
        content.push(_INTL("Nivel: {1}", pbBugContestState.lastPokemon.level))
        content.push(_INTL("Pokeballs: {1}", pbBugContestState.ballcount))
      else
